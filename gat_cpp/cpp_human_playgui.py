...
try:
    from env.human_play import HumanAIGameSession, ModelPolicyAdapter, StepResult, _import_mcts_cpp_module
except ModuleNotFoundError:
    from cpp_human_play import HumanAIGameSession, ModelPolicyAdapter, StepResult, _import_mcts_cpp_module
...
class CppAIAIAutoSession:
    """C++ 引擎下的 AI vs AI 会话。"""

    def __init__(self, board_size: int, n_in_row: int, c_puct=5.0, n_playout=240, cpp_threads=0):
        self.board_size = board_size
        self.n_in_row = n_in_row
        self.n_playout = n_playout
        ...
        self.board = self.mcts_cpp.Board(board_size, board_size, n_in_row)
        self.human_player_id = -1
        self.ai_player_id = 1

        self.ai_black = self.mcts_cpp.AlphaZeroPlayer(
            c_puct=int(c_puct),
            n_playout=int(n_playout),
            num_threads=int(cpp_threads),
        )
        self.ai_white = self.mcts_cpp.AlphaZeroPlayer(
            c_puct=int(c_puct),
            n_playout=int(n_playout),
            num_threads=int(cpp_threads),
        )
    ...
    def apply_ai_move(self, temp=1e-3):
        ...
        self.board.do_move(move)
        # 双方都同步最新一步，避免轮转时树漂移
        self.ai_black.update_with_move(move)
        self.ai_white.update_with_move(move)
...
class AIWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.move_finished.emit((result, elapsed_ms))
...
class GomokuWindow(QMainWindow):
    def __init__(
        ...
        self.default_cpp_threads = default_cpp_threads
        self.session = None
        self.mode = "human_ai"
    ...
    def init_ui(self):
        ...
        setup_row_engine = QHBoxLayout()
        self.label_engine = QLabel("3. 引擎后端")
        self.label_engine.setObjectName("Status")
        self.combo_engine = QComboBox()
        self.combo_engine.addItem("Python 模型引擎", "python")
        self.combo_engine.addItem("C++ 并行引擎", "cpp")
        engine_idx = self.combo_engine.findData(self.default_engine)
        self.combo_engine.setCurrentIndex(engine_idx if engine_idx >= 0 else 0)
        self.combo_engine.currentIndexChanged.connect(self.on_setup_changed)
        self.label_cpp_threads = QLabel("C++线程")
        self.label_cpp_threads.setObjectName("Status")
        self.combo_cpp_threads = QComboBox()
        self.combo_cpp_threads.addItem("自动", 0)
        for t in [1, 2, 4, 6, 8, 12, 16]:
            self.combo_cpp_threads.addItem(str(t), t)
        threads_idx = self.combo_cpp_threads.findData(self.default_cpp_threads)
        self.combo_cpp_threads.setCurrentIndex(threads_idx if threads_idx >= 0 else 0)
        self.combo_cpp_threads.currentIndexChanged.connect(self.on_setup_changed)
        setup_row_engine.addWidget(self.label_engine)
        setup_row_engine.addWidget(self.combo_engine)
        setup_row_engine.addWidget(self.label_cpp_threads)
        setup_row_engine.addWidget(self.combo_cpp_threads)
        layout.addLayout(setup_row_engine)

        setup_row3 = QHBoxLayout()
        self.label_model = QLabel("4. 黑棋模型")
        ...
        self.label_model_white = QLabel("5. 白棋模型(AI vs AI)")
    ...
    def set_setup_enabled(self, enabled: bool):
        ...
        self.combo_cpp_threads.setEnabled(enabled)
        ...
        if self.combo_engine.currentData() == "cpp":
    ...
    def on_setup_changed(self):
        ...
        is_cpp_engine = self.combo_engine.currentData() == "cpp"
        ...
        self.label_model.setVisible(not is_cpp_engine)
        self.combo_model_black.setVisible(not is_cpp_engine)
        self.btn_refresh_models.setVisible(not is_cpp_engine)
        self.label_cpp_threads.setVisible(is_cpp_engine)
        self.combo_cpp_threads.setVisible(is_cpp_engine)
        self._update_model_path_label()

    def _update_model_path_label(self):
        if self.combo_engine.currentData() == "cpp":
            threads = self.combo_cpp_threads.currentData()
            mode_text = "人机" if self.combo_mode.currentData() == "human_ai" else "AI vs AI"
            self.model_path_label.setText(f"当前选择: C++ 并行引擎 | 模式: {mode_text} | 线程: {threads}")
            return
    ...
    def _build_session_from_setup(self):
        ...
        if engine == "python" and not black_model_path:
        ...
        if mode == "human_ai":
            session = HumanAIGameSession(
                ...
                n_in_row=self.n_in_row,
                model_path=(black_model_path if engine == "python" else None),
                c_puct=5.0,
                n_playout=400,
                human_first=True,
                engine=engine,
                cpp_threads=int(self.combo_cpp_threads.currentData()),
            )
            human_first = True
        else:
            if engine == "python":
    ...
    def on_ai_finished(self, result):
        ...
        if elapsed_ms is not None:
            n_playout = getattr(self.session, "n_playout", None)
            if isinstance(n_playout, int) and n_playout > 0 and elapsed_ms > 0:
                pps = n_playout / (elapsed_ms / 1000.0)
                perf_suffix = f" | {elapsed_ms:.1f} ms | {pps:.0f} playout/s"
            else:
                perf_suffix = f" | {elapsed_ms:.1f} ms"

        if self.mode == "ai_ai" and self.game_running:
            self.update_status(f"AI vs AI 对局中...{perf_suffix}")
            QTimer.singleShot(60, self.trigger_ai_turn)
        else:
            self.update_status(f"你的回合{perf_suffix}")
...
def parse_args():
    ...
    parser.add_argument("--engine", type=str, default="python", choices=["python", "cpp"], help="默认引擎后端")
...
if __name__ == "__main__":
    ...
    window = GomokuWindow(
        ...
        default_cpp_threads=args.cpp_threads,
    )
    window.show()
    sys.exit(app.exec())